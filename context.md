...
This document reflects the current architectural direction and recent
design decisions.
...
-   Concrete logging providers are configured at application/bootstrap
    level.
...
-   This allows application-level customization without modifying the
    framework.
...
-   This avoids explicit construction of `NavigationParameters` in
    common scenarios.
...
For MAUI, a lightweight explicit registration-based locator is currently
used:
...
This keeps the framework small while allowing MAUI to stay
ViewModel-first without relying on Shell routing.
...
When a navigation guard returns `AskUser`, the result of the
confirmation dialog determines the outcome:
...
This enables scenarios like: - Save changes (True) - Discard changes
(False) - Cancel navigation (None)
...
The current direction is toward a unified shell-hosted dialog model
across platforms.
...
-   `MauiDialog` hosts the actual dialog content through an inner
    `ContentControl`.

This keeps dialogs inside the application's visual composition instead
of requiring a separate native window.

### WPF Direction

WPF supports two dialog hosting modes: shell overlay and separate window
hosting. - Overlay is the default mode. - Window hosting uses
WpfDialogWindow.
...
-   clicking a command passes its `DialogResult` back to the ViewModel
    through `ContinueAsync`
-   the callback returns whether the dialog should actually close
-   if no exchange info is provided, the host falls back to a default
    single `OK` command

This avoids old fixed button concepts like `YesNoCancel` while still
supporting validation-driven scenarios such as login dialogs or later
wizard-like flows.
...
Implements dialog completion infrastructure and acts as the common base
for dialog ViewModels such as `AboutViewModel` or `LoginViewModel`.
...
-   Dialogs are separate via `ShowDialogAsync(...)` (no modal flag on
    navigation).

### Target Identity

A navigation target is identified by: - `TargetType` - `NavigationKey`

Navigation to the same target is blocked only if **both match**.

Examples: - Same type, different key (e.g. different Id) → allowed -
...
Use a custom key when: - only a subset of parameters defines identity -
parameters contain complex objects - `ToString()` is not stable enough
...
Each entry stores: - Target instance - TargetType - Parameters -
NavigationKey

`GoBackAsync()` restores the original semantic target (type +
parameters + key).
...
If both are set: - The previous back stack is cleared - The current
entry is added as the sole back stack entry

This allows scenarios like: - Reset navigation history but still allow a
...
When navigation is blocked due to identical target identity (same type +
same `NavigationKey`):
...
Platform integration should be made as easy as possible, using a fluent
and consistent approach across platforms.

The core idea is:

> Each platform provides a lightweight IMvvmNavStarter that encapsulates
> all platform-specific bootstrapping.

This keeps the user-facing integration minimal while preserving full
flexibility.
...
The starter is resolved via DI and invoked at the appropriate lifecycle
...
WpfNavigationHostBuilder .Default() .WithServices(...) .WithLogging(...)
.WithShell\<ShellWindow, ShellViewModel\>()
.WithStartupNavigation`<HomeViewModel>`{=html}() .Build();

await host.StartAsync();

WPF does not use IMvvmNavStarter, because the builder already
encapsulates the startup process.
...
public override async void OnFrameworkInitializationCompleted() { var
starter = Services.GetRequiredService`<IMvvmNavStarter>`{=html}();
...
-   Application must have a parameterless constructor (platform
    constraint)
...
public partial class App : Application { private readonly
IMvvmNavStarter \_starter;
...
`<ContentControl Content="{Binding CurrentModule}" />`{=html}

The DataTemplate associated with the ViewModel resolves the View
automatically.
...
2.  **Use platform mechanisms**\
    The framework stays close to how the target UI platform wants to
    work.
...
6.  **Step-by-step evolution**\
    Architectural changes should be introduced incrementally and
    evaluated in sample applications before broader generalization.
...
-   The sample application is not only a demonstration, but actively
    drives architectural decisions.
-   Design choices are validated against real UI scenarios rather than
    theoretical abstraction.
-   This approach helps to avoid premature generalization.

### General Approach

-   Prefer a single conceptual sample application shared across
    platforms.
-   Platform-specific integration is demonstrated per platform:
    -   WPF: `WpfNavigationHostBuilder` + Serilog
    -   Avalonia: default builder
    -   MAUI: integration into existing app bootstrap with custom shell
        page
...
-   Feature detail pages load markdown files based on a navigation
    parameter key.
...
-   Test classes are to be named like the type to be tested with two
    trailing underscores, e.g. `DialogViewModelBase__`
-   When external references are necessary, they should be injected into
    the test class; here the `DIClassConstructor` is to be adapted and
    used
-   The comments `// Arrange`, `// Act` and `// Assert` are to be used
    and put together if those states overlap
...
# Packaging, MAUI & Debugging Notes (Project Learnings)

## Directory.Build.props & Multi-Target

Global TargetFramework settings can break multi-target projects
(especially MAUI).

Rule:

-   Do not globally enforce TargetFramework on multi-target projects
-   Prefer condition based on UseMaui:

``` xml
<TargetFramework Condition="'$(UseMaui)' != 'true'">net10.0</TargetFramework>
```

This avoids the need to maintain project name exclusions and scales
better.
...
-   Local feed used: `D:\dev\.nuget`
-   NuGet.config is stored at repository root

### Build Order

-   Abstractions
-   Core
-   Platform packages (Wpf, Avalonia, Maui)

### Key Insight

Code changes are not reflected automatically:

-   NuGet uses cached versions
-   Always rebuild and repack after changes
-   Prefer version suffixes for local work:
...
## MAUI Packaging Specifics

MAUI libraries behave differently from standard class libraries:

-   Must use platform-specific TFMs (android, ios, windows, etc.)
-   Do not include neutral `net10.0` in MAUI UI libraries
-   `dotnet pack` should be used without `--no-build`
...
SourceLink support: ON
```

### Important Insight

-   SourceLink allows inspecting source code
-   It does not replace Debug builds
-   Release optimizations still apply
...
Behavior differs significantly:
  ...
  Visual Studio            CLI
  ------------------------ ---------------------------
  Uses ProjectReferences   Uses PackageReferences
  Uses internal cache      Uses actual NuGet restore
...
Always validate outside the IDE.

------------------------------------------------------------------------

## Recovery / Reset Procedure

When encountering inconsistent or non-explainable errors:
...
This resolves many MAUI and multi-target related issues.

------------------------------------------------------------------------

## MAUI Build Model Insight

Even when code is platform-agnostic:

-   MAUI enforces platform-specific builds
-   Multi-targeting is required
-   Build complexity is independent from code simplicity
...
dotnet add package ADaxer.MvvmNav.Maui --source D:\dev\.nuget
dotnet build
```
...
-   package is usable in real projects
